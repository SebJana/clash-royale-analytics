...
export function usePageLoadingState({
  ...
  }, [isLoading, hasHadLoadingActivity]);
  ...
  // Reset initial load state when resetDependency changes (e.g., navigating to different player or different player page)
  useEffect(() => {
    if (resetDependency !== undefined) {
      setIsInitialLoad(true);
      setHasEverHadData(false);
      setHasCompletedLoad(false);
      setHasHadLoadingActivity(false);
      loadStartTimeRef.current = Date.now();
    }
  }, [resetDependency]);
